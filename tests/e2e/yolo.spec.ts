...
for (const fullFrame of [false, true]) {
  test(`follows an off-center cube and clears lost targets (${fullFrame ? 'full-frame YOLO' : 'sampling'})`, async ({
    page,
  }) => {
    const errors: string[] = [];
    page.on('pageerror', (error) => errors.push(error.message));
    ...
    if (fullFrame) {
      await page
        .getByLabel('色块识别 ONNX 模型')
        .setInputFiles(path.resolve('tests/fixtures/input-driven-frame-stickers-test.onnx'));
      await expect(page.locator('.model-line:not(.locator-line)')).toContainText('全画面识别');
    }
    await page.getByRole('button', { name: '开启摄像头' }).click();
    await expect(page.locator('.tracked-cube')).toBeVisible();
    await expect(page.getByRole('button', { name: '采集此面' })).toBeEnabled();
    if (fullFrame) await expect(page.locator('.detection-box')).toHaveCount(9);
    ...
    await page.screenshot({
      path: `test-results/auto-position-${fullFrame ? 'frame' : 'sampling'}.png`,
      fullPage: true,
    });
    await page.getByRole('button', { name: '采集此面' }).click();
    await expect(page.locator('.face-card.recorded')).toHaveCount(1);
